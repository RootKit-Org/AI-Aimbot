...
def main():
    ...
    # Loading Yolov8 Small AI Model. For ONNX or TensorRT use yolov8s.onnx or yolov8s.engine
    model = YOLO('yolov8s')
    ...
    while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
        ...
        pred = model(npImg, classes=0, conf=confidence, iou=.25, verbose=False)

        # Converting output to usable cords
        targets = []
        for detections in pred:
            for det in detections.boxes: #iterate over each detection                
                xyxy, confidence = det.xyxy[0].tolist(), float(det.conf[0].item())
                x1, y1, x2, y2 = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])
                targets.append([int((x1 + x2) / 2), int((y1 + y2) / 2), x2 - x1, y2 - y1])
